...
// bsModalMode: 'task' = completing a task, 'manual' = standalone log
let bsModalMode = 'manual';
...
// ─── BLOOD SUGAR MODAL ────────────────────────
// mode: 'task' (completing a BS task) | 'manual' (standalone log from Tasks tab)
function showBSModal(mode, taskId) {
  bsModalMode = mode || 'manual';
  document.getElementById('bs-task-id').value     = taskId || '';
  document.getElementById('bs-value-input').value = '';

  if (bsModalMode === 'manual') {
    document.getElementById('bs-modal-title').textContent = 'Log blood sugar';
    document.getElementById('bs-modal-desc').textContent  = 'Enter your current reading in mg/dL.';
    document.getElementById('bs-confirm-btn').textContent = 'Log reading';
    document.getElementById('bs-skip-btn').textContent    = 'Cancel';
  } else {
    document.getElementById('bs-modal-title').textContent = 'Log your reading';
    document.getElementById('bs-modal-desc').textContent  = 'Enter your blood sugar level in mg/dL.';
    document.getElementById('bs-confirm-btn').textContent = 'Log & complete';
    document.getElementById('bs-skip-btn').textContent    = 'Complete without logging';
  }
...
function confirmBSReading() {
  ...
  if (bsModalMode === 'task') {
    ...
    // Manual log — just save and refresh the graph
    renderBSGraph();
    const st = bsStatus(val);
    showToast(`🩸 ${val} mg/dL logged — ${st.label}`);
  }
}

function cancelBSModal() {
  const id   = document.getElementById('bs-task-id').value;
  hideBSModal();
  if (bsModalMode === 'task' && id) {
    const task = tasks.find(t => t.id === id);
    _completeDone(task);
  }
  // In manual mode, just close — nothing to complete
}
...
function renderBSGraph() {
  ...
  points.forEach((p, i) => {
    if (i % step === 0 || i === n - 1) ctx.fillText(p.label, toX(i), H - 6);
  });
}
...
function renderHistory() {
  ...
  history.forEach((entry, index) => {
    ...
    li.addEventListener('click', () => {
      if (confirm(`Remove "${entry.name}" from history?`)) deleteHistoryEntry(index);
    });
    list.appendChild(li);
  });
}
...
function sendNotification(title, body) {
  ...
  new Notification(title, { body, icon: 'https://em-content.zobj.net/source/apple/354/cactus_1f335.png' });
}
...
function init() {
  ...
  // Manual log button in Tasks tab
  document.getElementById('manual-bs-btn').addEventListener('click', () => {
    showBSModal('manual');
  });

  // Blood sugar modal buttons
  document.getElementById('bs-confirm-btn').addEventListener('click', confirmBSReading);
  document.getElementById('bs-skip-btn').addEventListener('click', cancelBSModal);
  document.getElementById('bs-value-input').addEventListener('keydown', e => {
    if (e.key === 'Enter')  confirmBSReading();
    if (e.key === 'Escape') cancelBSModal();
  });
  document.getElementById('bs-modal').addEventListener('click', function(e) {
    if (e.target === this) cancelBSModal();
  });
